...
task :parse_build_options do
  ...
  if ENV['TAILS_CROSS_ARM64_BUILD'].nil?
    ENV['TAILS_CROSS_ARM64_BUILD'] = '0'
  end
...
def list_artifacts
  ...
  target_architecture = ENV['TAILS_CROSS_ARM64_BUILD'] == '1' ? 'arm64' : ARCHITECTURE
  stdout = capture_vagrant_ssh("find '/home/#{user}/amnesia/' -maxdepth 1 " \
                                        "-name 'tails-#{target_architecture}-*' " \

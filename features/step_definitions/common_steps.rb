...
def boot_menu_images
  case @os_loader
  when 'UEFI'
    ['TailsBootMenuGRUB_Bookworm.png', 'TailsBootMenuGRUB_Bookworm_alt.png']
  else
...
Given /^the computer (?:re)?boots Tails$/ do
  ...
  # Deals with qemu bug https://bugzilla.redhat.com/show_bug.cgi?id=1661288 as discussed
  # therein and in https://gitlab.freedesktop.org/plymouth/plymouth/-/issues/121
  arm64_boot_options = \
    $config['ARM64'] ? 'console=tty0 plymouth.ignore-serial-consoles' : ''

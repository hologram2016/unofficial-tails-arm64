...
Then /^only the expected files are present on the persistence partition on USB drive "([^"]+)"$/ do |name|
  assert(!$vm.running?)
  # This could be defined either here or inside $vm.storage.guestfs_disk_helper
  # Since $vm.storage.guestfs_disk_helper may want to have access to the origin
  # backing file of disk, define it here
  if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS']
    potential_last_snapshot = $vm.storage.last_external_snapshot(name)

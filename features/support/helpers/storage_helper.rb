...
class VMStorage
  ...
  def clone_to_new_disk(from, to)
    ...
    if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS']
      potential_last_snapshot = last_external_snapshot(from)
      unless potential_last_snapshot.nil?
        from = potential_last_snapshot
      end
    end
  ...
  def last_external_snapshot(disk)
    potential_last_snapshot = "#{@pool_path}/#{disk}.last"
    return unless File.exist?(potential_last_snapshot)

    potential_last_snapshot = File.realpath(potential_last_snapshot)
    return if potential_last_snapshot.nil? || !File.exist?(potential_last_snapshot)

    File.basename(potential_last_snapshot)
  end
end

...
class VM
  ...
  def initialize(virt, xml_path, vmnet, storage, x_display)
    ...
    default_domain_xml_path = \
      $config['ARM64'] ? "#{@xml_path}/default-arm64.xml" : "#{@xml_path}/default.xml"
    default_domain_xml = File.read(default_domain_xml_path)
  ...
  def saved_image_rexml_desc(name)
    REXML::Document.new($virt.save_image_xml_desc(VM.external_snapshot_ram_path(name)))
  end

  def saved_image_xml_desc(name)
    saved_image_rexml_desc(name).to_s
  end
  ...
  def add_cdrom_device
    ...
    update do |xml|
      ...
      cdrom_xml_path = \
        $config['ARM64'] ? "#{@xml_path}/cdrom-scsi.xml" : "#{@xml_path}/cdrom.xml"
      cdrom_rexml = REXML::Document.new(
        File.read(cdrom_xml_path)
      ).root
  ...
  def saved_image_list_disk_devs(name)
    saved_image_xml = saved_image_rexml_desc(name)
    ret = []
    saved_image_xml.elements.each('domain/devices/disk') do |e|
      ret << e.elements['target'].attribute('dev').to_s
    end
    ret
  end
  ...
  def plug_drive(name, type)
    if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS']
      potential_last_snapshot = @storage.last_external_snapshot(name)
      name = potential_last_snapshot unless potential_last_snapshot.nil?
    end
  ...
  def disk_xml_desc(name)
    domain_xml.elements.each('domain/devices/disk') do |e|
      if e.elements['source'].attribute('file').to_s \
        ...
        return e.to_s
      elsif $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS'] && \
            disk_is_last_snapshot_of(e, @storage.disk_path(name))
        return e.to_s
      end
    rescue StandardError
      next
  ...
  def saved_image_disk_rexml_desc(image, name)
    saved_image_rexml = saved_image_rexml_desc(image)
    saved_image_rexml.elements.each('domain/devices/disk') do |e|
      if e.elements['source'].attribute('file').to_s == name
        return e
      end
    rescue StandardError
      next
  ...
  def saved_image_disk_xml_desc(image, name)
    disk_rexml_desc = saved_image_disk_rexml_desc(image, name)
    disk_rexml_desc&.to_s
  end

  def saved_image_disk_type(name, dev)
    saved_image_xml = saved_image_rexml_desc(name)
    saved_image_xml.elements.each('domain/devices/disk') do |e|
      if e.elements['target'].attribute('dev').to_s == dev
        return e.elements['driver'].attribute('type').to_s
  ...
  # This only works when the domain is active and 'disk'
  # is read from the domain's xml
  def disk_backing_store(disk)
    backing_store = disk.elements['backingStore']
    return if backing_store.nil?

    backing_store_source = backing_store.elements['source']
    return if backing_store_source.nil?

    backing_store_source_file = backing_store_source.attribute('file')
    return if backing_store_source_file.nil?

    backing_store
  end

  def disk_backing_store_file(disk, use_qemu_img: false)
    if use_qemu_img
      backing_store_file = \
        `qemu-img info "#{disk}" -U | grep -Po 'backing file:\\s\\K(.*)'`.chomp
      return if backing_store_file.empty?

      backing_store_file
    else
      backing_store = disk_backing_store(disk)
      backing_store.elements['source'].attribute('file').to_s if backing_store
    end
  end

  def disk_origin_store(disk)
    backing_store = disk_backing_store(disk)
    return disk if backing_store.nil? || backing_store == disk

    disk_origin_store(backing_store)
  end

  def disk_origin_store_file(disk, use_qemu_img: false)
    if use_qemu_img
      backing_store = disk_backing_store_file(disk, use_qemu_img: true)
      return disk if backing_store.nil? || backing_store == disk || \
                     File.basename(backing_store) == disk

      disk_origin_store_file(backing_store, use_qemu_img: true)
    else
      origin_store = disk_origin_store(disk)
      return if origin_store.nil?

      origin_store.elements['source'].attribute('file').to_s
    end
  end

  def disk_is_backed_by(disk, file)
    disk_origin_store_file(disk) == file
  end

  def disk_is_last_snapshot_of(disk, file)
    last_snapshot = File.realpath("#{file}.last")
    return false if last_snapshot.nil?

    disk_source = disk.elements['source']
    return false if disk_source.nil?

    disk_source_file = disk_source.attribute('file')
    return false if disk_source_file.nil?

    disk_source_file.to_s == last_snapshot
  end
    ...
    update do |xml|
      ...
      xml.elements.each("domain/devices/disk[@device='disk']") do |e|
        ...
        next unless source_file == @storage.disk_path(name) || \
                    ($config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS'] && \
                    disk_is_last_snapshot_of(e, @storage.disk_path(name)))
  ...
  def external_snapshot_xml(name)
    disk_devs = list_disk_devs
    disks_xml = "    <disks>\n"
    disk_devs.each do |dev|
      dev_disk_type = disk_type(dev)
      snapshot_type = dev_disk_type == 'qcow2' ? 'external' : 'no'
      dev_xml_content = if dev_disk_type == 'qcow2'
                          "<source file='#{VM.external_snapshot_dev_path(name, dev)}'/>"
                        else
                          ''
                        end
      disks_xml +=
        "      <disk name='#{dev}' snapshot='#{snapshot_type}'>" \
        "#{dev_xml_content}</disk>\n"
    end
    disks_xml += '    </disks>'
    <<~XML
      <domainsnapshot>
        <name>#{name}</name>
        <description>Snapshot for #{name}</description>
        <memory snapshot='external' file='#{VM.external_snapshot_ram_path(name)}'/>
      #{disks_xml}
        </domainsnapshot>
    XML
  end
  ...
  def self.external_snapshot_ram_path(name)
    "#{$config['TMPDIR']}/#{name}-snapshot.ext.memstate"
  end

  def self.external_snapshot_dev_path(name, dev)
    "#{$vmstorage.pool_path}/#{name}-#{dev}-snapshot.qcow2"
  end
  ...
  def save_external_snapshot(name)
    # Needs error recovery
    xml = external_snapshot_xml(name)
    flags = Libvirt::Domain::Snapshot::CREATE_NO_METADATA | \
            Libvirt::Domain::Snapshot::CREATE_HALT
    execute('sync')
    @domain.suspend
    @domain.snapshot_create_xml(xml, flags)
    saved_image_rexml = saved_image_rexml_desc(name)
    saved_image_rexml.elements.each('domain/devices/disk') do |disk|
      next unless disk.elements['driver'].attribute('type').to_s == 'qcow2'

      snapshot_dev_path = VM.external_snapshot_dev_path(name,
                                                        disk.elements['target']
                                                            .attribute('dev').to_s)
      disk.elements['source'].add_attribute('file', "#{snapshot_dev_path}.active")
      disk.delete_element('backingStore')
      snapshot_dev_backing_file = disk_backing_store_file(snapshot_dev_path,
                                                          use_qemu_img: true)
      next unless snapshot_dev_backing_file.end_with?('.active')

      snapshot_dev_backing_file = snapshot_dev_backing_file.delete_suffix('.active')
      system(
        "qemu-img rebase -b #{snapshot_dev_backing_file} -F qcow2 #{snapshot_dev_path}",
        out: File::NULL
      )
      File.delete("#{snapshot_dev_backing_file}.active")
    end
    $virt.define_save_image_xml(VM.external_snapshot_ram_path(name),
                                saved_image_rexml.to_s)
    @storage.refresh
  end
  ...
  def save_snapshot(name)
    ...
    if internal_snapshot
      if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS']
        save_external_snapshot(name)
        restore_external_snapshot(name)
      else
  ...
  def restore_snapshot(name)
    ...
    @display.stop if @display&.active?
    if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS']
      potential_external_snapshot_ram_path = VM.external_snapshot_ram_path(name)
      if File.exist?(potential_external_snapshot_ram_path)
        restore_external_snapshot(name)
        @display.start
        return
      end
  ...
  def restore_external_snapshot(name)
    disk_devs = saved_image_list_disk_devs(name)
    disk_devs.each do |dev|
      next unless saved_image_disk_type(name, dev) == 'qcow2'

      source = VM.external_snapshot_dev_path(name, dev)
      dest = "#{source}.active"
      system("qemu-img create -b #{source} -f qcow2 -F qcow2 #{dest}",
             out: File::NULL)
      marker_file = "#{disk_origin_store_file(dest, use_qemu_img: true)}.last"
      if File.exist?(marker_file) || File.symlink?(marker_file)
        File.delete(marker_file)
      end
      File.symlink(File.basename(dest), marker_file)
    end
    @storage.refresh
    @domain = @virt.define_domain_xml(saved_image_rexml_desc(name).to_s)
    Libvirt::Domain.restore($virt, VM.external_snapshot_ram_path(name))
    @domain = @virt.lookup_domain_by_name(@domain_name)
    @domain.resume
    JournalDumper.instance.start
  end
  ...
  def self.remove_snapshot(name)
    if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS']
      potential_external_snapshot_ram_path = VM.external_snapshot_ram_path(name)
      if File.exist?(potential_external_snapshot_ram_path)
        File.delete(potential_external_snapshot_ram_path)
        Dir.glob("#{$vmstorage.pool_path}/#{name}-*-snapshot.qcow2*").each do |file|
          File.delete(file)
        end
        return
      end
  ...
  def self.snapshot_exists?(name)
    ...
    return true if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS'] && \
                   File.exist?(VM.external_snapshot_ram_path(name))
  ...
  def self.remove_all_snapshots
    ...
    if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS']
      Dir.glob("#{$config['TMPDIR']}/*-snapshot.ext.memstate").each do |file|
        # $vmstore may not be initialized yet, so we must look for
        # disk snapshots manually
        REXML::Document.new($virt.save_image_xml_desc(file))
                       .elements.each('domain/devices/disk') do |e|
          disk_source = e.elements['source']
          next if disk_source.nil?

          disk_file = disk_source.attribute('file')
          next if disk_file.nil?

          disk_dir = File.dirname(disk_file.to_s)
          # This may also delete snapshots not associated to 'file', but we are fine
          # with it as we are deleting all snapshots anyway
          Dir.glob("#{disk_dir}/*-snapshot.qcow2*").each do |disk_snapshot|
            File.delete(disk_snapshot)
          end
        end
        File.delete(file)
      end

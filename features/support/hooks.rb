...
AfterFeature('@product') do
  ...
  vols_del = $vmstorage.list_volumes
                       .reject do |vol_name|
    vol_name == '__internal' || \
      ($config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS'] && \
       vol_name.end_with?('-snapshot.qcow2'))
  end
  vols_del.each { |vol_name| $vmstorage.delete_volume(vol_name) }
  if $config['LIBVIRT_USE_EXTERNAL_SNAPSHOTS']
    Dir.glob("#{$vmstorage.pool_path}/*.last").each do |lastfile|
...
Before('@product') do |scenario|
  ...
  $language = ''
  $lang_code = ''
  @os_loader = $config['ARM64'] ? 'UEFI' : 'MBR'

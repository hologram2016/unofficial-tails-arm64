...
do
    case "$1" in
        ...
        --flavor|--flavour|--network-flavor|--network-flavour)
            export BOOTSTRAP_ARG="--network-flavor=$2"
            shift
        ;;
        --network-script)
            export BOOTSTRAP_ARG="--network-script=$2"
            shift
        ;;
...
if [ -z "${CHUTNEY_CONTROLLING_PID+x}" ]; then
    if [ "${CHUTNEY_START_TIME:-1}" -ge 0 ] \
       && [ "${CHUTNEY_BOOTSTRAP_TIME:-1}" -ge 0 ] \
       && [ "${CHUTNEY_STOP_TIME:-1}" -ge 0 ]; then
        # exit when this script exits
        export CHUTNEY_CONTROLLING_PID="$$"
    fi
fi
...
# mandatory: $CHUTNEY_PATH is the path to the chutney directory
# if it's not set:
...
if [ ! -d "$CHUTNEY_PATH" ] || [ ! -x "$CHUTNEY_PATH/chutney" ] || \
    ...
    if [ -x "$PWD/chutney" ] && [ -f "$PWD/chutney" ]; then
        $ECHO "$myname: \$CHUTNEY_PATH is not a valid chutney directory, trying \$PWD"
        export CHUTNEY_PATH="$PWD"
    elif [ -d "$TOOLS_DIR/.." ] && \
         [ -x "$TOOLS_DIR)/../chutney" ] && \
         [ -f "$TOOLS_DIR/../chutney" ]; then
        $ECHO "$myname: \$CHUTNEY_PATH is not a valid chutney directory, using this script's location"
        export CHUTNEY_PATH="$TOOLS_DIR/.."
    elif [ -d "$TOR_DIR" ] && \
	 [ -d "$TOR_DIR/../chutney" ] && \
         [ -x "$TOR_DIR/../chutney/chutney" ] && \
	 [ -f "$TOR_DIR/../chutney/chutney" ]; then
        $ECHO "$myname: \$CHUTNEY_PATH is not a valid chutney directory, trying \$TOR_DIR/../chutney"
...
# set CHUTNEY_DATA_DIR if user hasn't.
if [ -z "${CHUTNEY_DATA_DIR:+x}" ]; then
    # We ensured above that CHUTNEY_PATH is set
...
# Set the variable for the chutney network flavour
